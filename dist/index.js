...
var GENERATED_NOTICE = "Autogenerated by `agent-journal --dump`. Do not edit: changes are overwritten on the next dump.";
function iso(ms) {
...
function resetMarkdownDir(dir) {
  fs4.mkdirSync(dir, { recursive: true });
  for (const file of fs4.readdirSync(dir)) {
    if (file.endsWith(".md")) {
      fs4.rmSync(path5.join(dir, file));
    }
  }
}
...
function statementMarkdown(statement) {
  const headline = singleLine(statement.claim);
  const truncated = headline.length > 80;
  const lines = [`### ${truncated ? `${headline.slice(0, 80)}\u2026` : headline}`];
...
function entityMarkdown(entity, statements) {
...
function journalDayMarkdown(day, entries, links) {
  const lines = ["---", `# ${GENERATED_NOTICE}`, `date: ${day}`, "---", "", `# Journal \u2014 ${day}`];
...
function relationshipsMarkdown(relationships) {
  const lines = [`<!-- ${GENERATED_NOTICE} -->`, "", "# Relationships", ""];
  for (const relationship of relationships) {
    const invalid = relationship.status !== "active" ? " (invalid)" : "";
    lines.push(
      `- ${relationship.id}: ${relationship.from_entity} \u2014${singleLine(relationship.type)}\u2192 ${relationship.to_entity} \xB7 ${relationship.confidence_level}${invalid}`
    );
  }
  return `${lines.join("\n")}
`;
}
function dumpMemory(db, resolver, baseDir) {
  const project = resolver.resolve();
  const kbDir = path5.join(baseDir, ".agents", "knowledge-base");
  const journalDir = path5.join(baseDir, ".agents", "journal");
  resetMarkdownDir(kbDir);
  resetMarkdownDir(journalDir);
  const entities = db.prepare("SELECT * FROM entity WHERE project_id = ? ORDER BY created_at, id").all(project.id);
  const statements = db.prepare("SELECT * FROM statement WHERE project_id = ? ORDER BY created_at, id").all(project.id);
  const statementsByEntity = /* @__PURE__ */ new Map();
  ...
  for (const entity of entities) {
    ...
    fs4.writeFileSync(path5.join(kbDir, fileName), entityMarkdown(entity, statementsByEntity.get(entity.id) ?? []));
    filesWritten += 1;
  }
  const relationships = db.prepare("SELECT * FROM relationship WHERE project_id = ? ORDER BY created_at, id").all(project.id);
  if (relationships.length > 0) {
    fs4.writeFileSync(path5.join(kbDir, "relationships.md"), relationshipsMarkdown(relationships));
    filesWritten += 1;
  }
  const journalEntries = db.prepare("SELECT * FROM journal_entry WHERE project_id = ? ORDER BY created_at, id").all(project.id);
  ...
  const linksByJournal = /* @__PURE__ */ new Map();
  ...
  for (const [day, entries] of entriesByDay) {
    fs4.writeFileSync(path5.join(journalDir, `${day}.md`), journalDayMarkdown(day, entries, linksByJournal));
    filesWritten += 1;
  }
  return {
    kbDir,
...
var MemoryApi = class {
  ...
  stats(input) {
    ...
    return {
      ...
      db_file_size: fs5.existsSync(this.dbFile) ? fs5.statSync(this.dbFile).size : 0,
...
function runDump() {
  ...
  try {
    ...
    process.stdout.write(
      `Dumped ${result.entities} entities with ${result.statements} statements, ${result.journalEntries} journal entries${result.relationships > 0 ? `, ${result.relationships} relationships` : ""} (${result.filesWritten} files).
  ${path6.relative(process.cwd(), result.kbDir) || result.kbDir}
  ${path6.relative(process.cwd(), result.journalDir) || result.journalDir}
`
    );
  } finally {
    db.close();
